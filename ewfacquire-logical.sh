...
EXAMINER="$5"
if [ $# -eq 0 ]; then
	USAGE "DEVICE" "OFFSET" "BASE DEST. NAME" "CASE #" "EXAMINER" && exit 1
fi

ewfacquire -o "$OFFSET" -M logical -P 4096 -u -c deflate:best -C "$CASE" -e "$EXAMINER" -f encase6 -l "$DEST-ewfacquire.log" -t "$DEST" "$DEVICE"
